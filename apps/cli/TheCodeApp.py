...
def modifSecurite():
    """Donne la securite suivant le nombre de bits"""
    ...
    elif bits < 64 : secure = " Très Faible "; couleur = "red"
    ...
    else : secure = " Très Forte "; couleur = "green"
...
def clefFichier(fichier_source):
    """Renvoie valeur de fichier source"""

    with open (fichier_source, 'rb') as f:
        clef = f.read()
    return clef.decode('UTF-8','ignore')

## Fonctions pour l'application

def Generer(clef = None, event = None, event2 = None) :
    ...
    if clef == None or clef == "PY_VAR2" or clef == "PY_VAR0":
        clef = clefEntry.get()
...
def ouvrirFichier():
    fichier_entree = filedialog.askopenfilename(title = "Choisissez un fichier qui servira de clef")

    try:
        clef = clefFichier(fichier_entree)
        Generer(clef)
...
def DarkMode():
    ...
    ouvrir.config(highlightbackground = back)
...
ouvrir = tk.Button(root, text = 'Ouvrir', command = ouvrirFichier)
...
fenetre.create_window(350, begin, window = ouvrir)
begin += 30
